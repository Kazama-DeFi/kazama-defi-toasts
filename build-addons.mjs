  ...
  for (const dir of dirs) {
    ...
    for (const moduleType of ['cjs', 'esm']) {
      ...
      const { stdout, stderr } = await asyncExec(
        `${MICRO_BUNDLE} -i ${entryPoint} -o ${out} --no-pkg-main -f ${moduleType} --jsx React.createElement --external kazama-defi-toasts --compress false --tsconfig tsconfig.build.json`
      );
      console.log(stdout);
      console.log(stderr);
